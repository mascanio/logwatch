...
func (m *Model) SetContent(s []string) {
	m.lines = make([]string, len(s))
	for _, str := range s {
		m.lines = append(m.lines, strings.ReplaceAll(str, "\r\n", "\n"))
	}
...
func (m Model) IsMoveMsg(msg tea.Msg) MoveDir {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch {
		case key.Matches(msg, m.KeyMap.PageDown),
			...
			return MoveDown
		case key.Matches(msg, m.KeyMap.PageUp),
			key.Matches(msg, m.KeyMap.HalfPageUp),
